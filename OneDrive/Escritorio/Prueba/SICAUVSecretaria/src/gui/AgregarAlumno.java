...
public class AgregarAlumno extends javax.swing.JFrame {
    ...
    public AgregarAlumno(int idFacultad) {
        initComponents();
    ...
    public static byte[] convertirtFileContentToBlob(String filePath) throws IOException {
    ...
    private void selecFotoButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selecFotoButton1ActionPerformed
        ...
        if (Buscar.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            RutaFoto = Buscar.getSelectedFile().toString();
            Image Foto = Tool.createImage(RutaFoto);
            firmaLabel.setIcon(new ImageIcon(Foto.getScaledInstance(firmaLabel.getWidth(), firmaLabel.getHeight(), Image.SCALE_AREA_AVERAGING)));
        }
    ...
    private void selecFotoButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selecFotoButtonActionPerformed
        ...
        if (Buscar.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            Toolkit Tool = Toolkit.getDefaultToolkit();
            RutaFirma = Buscar.getSelectedFile().toString();
            Image Firma = Tool.createImage(RutaFirma);
            fotoLabel.setIcon(new ImageIcon(Firma.getScaledInstance(fotoLabel.getWidth(), fotoLabel.getHeight(), Image.SCALE_AREA_AVERAGING)));
        }
            ...
            if (nombreFacultad.length() == 0) {
                ...
                for (IFacultad iFacultad : listaFacultad) {
                    if (iFacultad.getFacultad().equals(nombreFacultad)) {
                        ...
                        /*try {
                            estudiante.setFoto(Imagenes.readFile(RutaFoto));
                        } catch (SQLException ex) {
                            Logger.getLogger(AgregarAlumno.class.getName()).log(Level.SEVERE, null, ex);
                        }
                        try {
                            estudiante.setFirma(Imagenes.readFile(RutaFirma));
                        } catch (SQLException ex) {
                            Logger.getLogger(AgregarAlumno.class.getName()).log(Level.SEVERE, null, ex);
                        }*/
                    }
